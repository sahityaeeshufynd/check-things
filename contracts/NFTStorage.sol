...
contract MyNFTStorage is ERC1155Holder {
    ...
    //================LISTING MAPPINGS=========================
    mapping(address => mapping(uint => mapping(address => uint)))
        private tokenBalance; //nftAddress -> tokenId -> userAddress -> quantity
    mapping(address => mapping(uint => mapping(address => Token)))
        private tokenIdToUserToTokenInfo; //nftAddress -> userAddress -> TokenObject

    function _listUserDefinedNFTForSale(
        address nftAddress,
        uint tokenId,
        uint tokenPrice,
        address account,
        ...
        //please ensure that this remains internal
        require(
        ...
        Token storage myToken = tokenIdToUserToTokenInfo[nftAddress][tokenId][
            account
        ];
        ...
        myToken.tokenPrice =
            (myToken.tokenPrice * (10000 + feePercent)) /
            10000;
    ...
    function _listKraftNFTForSale(
        ...
        kraftnft = KraftNFT(nftAddress);
        uint tokensHeld = kraftnft.balanceOf(account, tokenId);
        require(
            tokensHeld > 0,
            "NFT Storage: This user does not have any units of this token available for listing!"
        );
        require(
            quantity <= tokensHeld,
            "You cannot list this units of this token, try reducing the quantity!"
    ...
    function _claimToken(
        ...
        Token storage ownerToken = tokenIdToUserToTokenInfo[nftAddress][
            tokenId
        ][owner];
        ...
        Token storage newOwner = tokenIdToUserToTokenInfo[nftAddress][tokenId][
            account
        ];
    ...
    function _withdrawNFT(
        ...
        require(
            quantity <= myToken.quantity,
            "You do not have sufficient tokens, withdraw less!"
        );
    ...
    function getTokenPrice(
        ...
        return
            tokenIdToUserToTokenInfo[nftAddress][tokenId][account].tokenPrice;
    }
    ...
    function verifyQuantityIsAvailable(
        address nftAddress,
        uint tokenId,
        address tokenOwner,
        uint quantity
